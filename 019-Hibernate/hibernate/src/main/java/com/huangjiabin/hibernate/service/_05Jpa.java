...
//        em.remove(customer);
        ...
        //正确，只能删除持久态
        Customer customer = em.find(Customer.class, 18L);
        em.remove(customer);
    ...
    public void test_jpql(){
        ...
        String jpql = "UPDATE com.huangjiabin.hibernate.po.Customer set custName = :name where custId = :id";
        em.createQuery(jpql)
                .setParameter("name","王二")
                .setParameter("id",99L)
                .executeUpdate();
        /*第二种传参方式
        String jpql = "UPDATE com.huangjiabin.hibernate.po.Customer set custName = ?1 where custId = ?2";
        em.createQuery(jpql)
                .setParameter(1,"王二")
                .setParameter(2,5L)
                .executeUpdate();
        */
        /*第三种传参，这种只能用在@Query注解上
            ...
            Customer findCustomerByInfo(@Param("customer") com.huangjiabin.hibernate.po.Customer customer);

        */
        /*
        org.hibernate.QueryException：不再支持旧式查询参数（`?`）；改用 JPA 风格的序数参数（例如，`?1`）：
        String jpql = "UPDATE com.huangjiabin.hibernate.po.Customer set custName = ? where custId = ?";
        em.createQuery(jpql)
                .setParameter(0,"王二")
                .setParameter(1,5L)
                .executeUpdate();
